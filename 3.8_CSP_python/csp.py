...
lunch_domains = [("breakfast", "Mon"), ("breakfast", "Tue"), ("lunch", "Mon"),
                 ("lunch", "Tue"), ("dinner", "Mon"), ("dinner", "Tue")]
...
def member_pred(sim_state):
    ...
    assigned_doms = [
        sim_state.names[x] for x in list_of_names if sim_state.names[x] != []
    ]
    letters_at_where = []
    for i in range(0, len(list_of_names)):
        letters_at_where = letters_at_where + list(
            zip(list_of_names[i], assigned_doms[i]))
    check = []
...
def master_pred(sim_state):
    ...
    if False in [member_pred(sim_state), length_pred(sim_state)]:
        return False
    else:
        return True
...
def assign_empty(cue, doms):
    ...
    new_cue = map(
        lambda x: sim_state.set('names', sim_state.names.set(empty_name, x)),
        doms)
...
def main_loop():
    ...
    final = ""
    solition = []
    domains = word_search_prep(
        ["anker", "Scott", "Bobby", "zone", "gopro", "polar"], 6)
    while stopper:
        c_head = the_cue[0]
        if goal_pred(c_head):
            stopper = False
